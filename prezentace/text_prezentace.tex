zahájeni
{
    ...
    původ
    název práce
}-> Dobrý den, jmenuji se Tomáš Vavrinec, pocházím z Brna a dnes vám budu prezentovat 
    svojí práci SOČ s názvem Postav si svého druhého robota % popřípadě ještě změnit to jméno
...
úvod
{
    ...
    výukou robotiky v kroužcích. Z toho důvodu potřebuji vhodné stavebnice se kterými 
    ...
    byla diskvalifikována kvůli technikáliím). Dalším důvodem vedle výuky byla možnost 
    ...
    do jedné ruky, což jsem vyřešil úplným oddělením dveří od těla trezoru a umožnil 
...
popis B.B.
{
    ...
    E4
        ...
        Software
            spolupracující SOČ, software a knihovni pro firnware vyvinul Tomáš Rohlínek 
                    v jazyce C++
    ...
    dvěma jardy procesoru Xtensa LX6, 
    což je dvaatřicetibitoví procesor, který běží na taktu 240MHz. 
    Čip ESP32 je zde uvnitř modulu wrover, konkrétně tento modul jsem zvolil pro dodatečnou 
    PSRam, která rozšiřuje REM o 8MB což je 16ti násobek původní velikost.
    Čip také disponuje Wifi a bluetooth což jsou pro mě velice důležitými funkcemi a jsou 
    jedním z hlavních důvodu volby tohoto čipu.
    ...
    Zařízení má k dispozici rozmanité senzory, a to
        ...
        Dalšími senzory jsou akcelerometr, gyroskop a magnetický která na první verzi 
        trezoru zajišťoval čip BMX055 ale pro jeho obtížnou dostupnost jsem v další verzi 
        k tomuto čipu přidal čipy MPU6050 a QMC5883 z nichž 
        MPU zajišťuje akcelerometr s gyroskope a QMC magnetický kompas.
        ...
        signalizačního kruhu. Čip LDC1614 je totiž schopen velmi přesný měřit indukčnost, na desce má 
        ...
        Tlaková plocha je také schopna bez problému odolat i ráně pěstí což je u herního prvku velice užitečné
        jelikož děti i někteří dospělí v zápalu hry často používají vetší sílu než je potřeba.

    Celí sytém je napájen dvěma paralelně zapojenými li-on články 18650, systém je ale sto bez omezení pracovat i s jedním článkem, 
    dva články jsou zde čistě kvůli kapacitě a využití volného místa.
    
    Přímo z baterii je ale napájen jen IR přijímač, trezor je totiž schopen komunikovat i po IR,
    a čip M41T62 neboli RTC který se nikdy nevypíná protože uchovává aktuální čas.
    Pro všechny ostatní periferie je zde lineární stabilizátor respektive stepup konvertor, pro napájecí větve 3.3 a 5V.
    Stepup napájí primárně světelný kruh ale je s nej napájen i motor zámku aby se pokud možno choval stejně bez ohledu na stav baterii.
    ...
    Protože trezor by nemělo být snadné přeprogramovat ve chvíli kdy uživatel nechce je tento obvod vybaven 
    i systémem který je schopen zakázat možnost využití programátoru, zároveň tento obvod šetří energii protože 
    převodníku poskytne napájení jen ve chvíli kdy je připojen programovací kabel je realizován pomocí USB-C konektoru

    Všechny tyto systémy jsou rozloženy na dvou respektive třech deskách.
    Na jedné desce je jen světelná kruh a LDC spolu se snímacími cívkami.
    Na druhé desce je vše ostatní, neurčitý počet desek je způsobeno 
    enkodérem který je realizován jako vylomitelná část hlavní desky.

    Elektroniku jsem původě navrhoval v programu Eagle ale pozdeji jsem přešel na program KiCad kvůli KiKitu což je nástroj 
    v kommant lině který zásadně zjednodušuje výrobu podkladů pro výrobu desek.

    Mechanické části jsem pak navrhoval v programu fusion 360 ve kterém jiš dlouho pracuji a který umožňuje jednoduchý 
    export různých podkladu pro výrobu na CNC strojích.
    První verze systému jsem pak vyráběl kombinací technologii laserové řezačky a 3D tisku jak technologii FDM tak SLA.
    Po odzkoušení konstrukce jsem přešel na variantu odlévání s polyuretanu. Tuto část v textu mé práce nenajdete jelikož 
    to není elektronika a práce mi už tak přišla dlouhá.

    Moje práce by nikdy nebyla funkční bez softwaru, který ale není součástí mojí práce. Je totiž obsahem samostatné práce SOČ
    s názvem SDK pro výuku programování na IoT stavebnici, kterou vypracoval můj kolega Tomáš Rohlínek a soutěží s ní v Informatiky.
    Napsal ale knihovny v jazyce C++ pro snadnou manipulaci se všemi periferiemi.
...
využití a zhodnocení
{
    ...
    E4
        pandemie
}-> Kvůli jednoduchosti byla mechanická verze trezoru hotova mnohem dřív než verze elektronická, tdy ještě před započetím světové pandemie.
    Z tohoto důvodu se stihla stát náplní jednoho kroužku který jsem spoluvedl, když byl hotový nahradila do třetí varianta elektronického 
    trezoru ale kvůli pandemii byla kroužek po pouhých dvou schůzkách přerušen a na dálku jsme nebyli sto pokračovat v jeho stavbě.
